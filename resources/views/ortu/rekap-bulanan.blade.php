...
@section('page-title', 'Rekap Presensi Bulanan')
...
.cal-day.cal-has-data { background: rgba(59,130,246,.10); font-weight: 700; }
...
<div class="cal-dashboard-grid">
    ...
    <form action="{{ route('ortu.rekap-bulanan') }}" method="GET" id="calForm" style="margin:0;">
        ...
        <input type="hidden" name="id_siswa" id="hiddenSiswa" value="{{ $selectedSiswa->id_siswa }}">
...
<div class="card">
    <div class="card-header" style="display:flex; justify-content:space-between; align-items:center;">
        <h3 class="card-title">
            Detail Presensi Harian {{ request('tanggal') ? '(' . \Carbon\Carbon::parse(request('tanggal'))->format('d/m/Y') . ')' : '' }}
        </h3>
        @if(request('tanggal'))
            <a href="{{ route('ortu.rekap-bulanan', ['id_siswa' => $selectedSiswa->id_siswa, 'bulan' => $bulan, 'tahun' => $tahun]) }}" class="btn btn-secondary btn-sm">Tampilkan Semua Hari Bulan Ini</a>
        @endif
    </div>
    <div class="card-body" style="padding:0;">
        ...
        <div class="empty-state">
            <div class="empty-state-text">Tidak ada data presensi pada periode/tanggal ini.</div>
        </div>
        @endif
    </div>
</div>
@else
<div class="alert alert-warning"><div>Data anak belum ditemukan atau belum terhubung dengan akun anda.</div></div>
@endif
