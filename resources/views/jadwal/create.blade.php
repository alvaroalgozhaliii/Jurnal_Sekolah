...
@section('content')
<h2>Tambah Jadwal Pelajaran Baru</h2>
...
<form action="{{ route('jadwal.store') }}" method="POST">
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                <select name="id_kelas" required style="width:300px; padding:5px;">
                    ...
                    @foreach($kelas as $k)
                    <option value="{{ $k->id_kelas }}" {{ (old('id_kelas') ?? $selectedKelasId) == $k->id_kelas ? 'selected' : '' }}>{{ $k->nama_kelas }}</option>
                    @endforeach
                </select>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <select name="id_guru" style="width:300px; padding:5px;">
                    ...
                    @foreach($guru as $g)
                    <option value="{{ $g->id_guru }}" {{ old('id_guru') == $g->id_guru ? 'selected' : '' }}>{{ $g->nama }} ({{ $g->bidang_studi ?? '-' }})</option>
                    @endforeach
                </select>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <select name="mapel" required style="width:300px; padding:5px;">
                    ...
                    @foreach($mapel as $mp)
                    <option value="{{ $mp->nama_mapel }}" {{ old('mapel') == $mp->nama_mapel ? 'selected' : '' }}>{{ $mp->nama_mapel }} ({{ $mp->kode_mapel ?? '-' }})</option>
                    @endforeach
                </select>
            </td>
        </tr>
        <tr>
            ...
            <td><input type="text" name="ruang" value="{{ old('ruang') }}" placeholder="Contoh: R.01" style="width:150px; padding:5px;"></td>
        </tr>
        <tr>
            <td></td>
            <td><button type="submit" style="padding:8px 20px; margin-top:10px;">SIMPAN JADWAL</button></td>
        </tr>
    </table>
</form>
