...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">Daftar penugasan Waka Bertugas dan Guru Piket harian</p>
    </div>
            ...
            <table class="table">
                ...
                @foreach($jadwal as $index => $item)
                    @php
                        ...
                        $isSayaPiket = ($guru && $item->id_guru_piket == $guru->id_guru);
                    @endphp
                    <tr style="{{ $isSayaPiket ? 'background:rgba(37,99,235,0.08); font-weight:600;' : ($isToday ? 'background:rgba(22,163,74,0.07);' : ($isWeekend ? 'background:rgba(245,158,11,0.04);' : '')) }}">
                        ...
                        <td>
                            @if($item->guruPiket)
                                <span class="fw-bold {{ $isSayaPiket ? 'text-primary' : '' }}">{{ $item->guruPiket->nama }}</span>
                                <div class="text-muted" style="font-size:11.5px;">
